...
namespace Magaz
{
    ...
    public class Market
    {
        ...
        public Market()
        {
            ...
            {
                new Product("Кетчуп", 250),
                new Product("Лимонад", 100),
                new Product("Курица", 500),
                new Product("Багет", 80)
            };

            _customersQueue = new ConcurrentQueue<Customer>();

            _uniqueNames = new List<string> { "Стасян", "Боря", "Филлипп", "Михаил", "Натаха", "Маша" };
        }
        ...
        private Product ChooseProduct(Customer customer)
        {
            ...
            {
                Console.WriteLine($"{i + 1}. {_products[i].Name} - {_products[i].Price} рублей");
            }
...
Console.WriteLine($"{_name} (№{_id}) купил {quantity} {product.Name}(-ов) на сумму {totalCost} рублей.");
            _money -= totalCost;
            Console.WriteLine($"У {_name} (№{_id}) осталось {_money} рублей.\n");
        }
    }
